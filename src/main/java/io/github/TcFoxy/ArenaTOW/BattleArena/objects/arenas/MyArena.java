...
public class MyArena extends AreaContainer {
...
//    }
     ...
     */
    public void setMatch(MyMatch myMatch) {
        this.match = myMatch;
    }
...
//    }
    ...
    public boolean matches(final MyJoinOptions jos) {
        MyArena a = jos.getArena();
        if (a != null && !a.matches(this))
            return false;
        MatchParams matchParams = jos.getMatchParams();
        if (!matches(matchParams)) {
            return false;
        }
        if (matchParams.hasOptionAt(MatchState.PREREQS, TransitionOption.WITHINDISTANCE)) {
            if (!jos.nearby(this, matchParams.getDoubleOption(MatchState.PREREQS, TransitionOption.WITHINDISTANCE))) {
                return false;
            }
        }
        if (matchParams.hasOptionAt(MatchState.PREREQS, TransitionOption.SAMEWORLD)) {
            if (!jos.sameWorld(this)) {
                return false;
            }
        }
    ...
    public boolean matches(MyArena arena) {
        if (arena == null)
            return false;
        if (this == arena)
            return true;
...
//            return false;
    ...
    public List<String> getInvalidMatchReasons(MyArena arena) {
        List<String> reasons = new ArrayList<String>();
        if (arena == null){
    ...
    public List<String> getInvalidMatchReasons(MatchParams matchParams, MyJoinOptions jp) {
        List<String> reasons = new ArrayList<String>();
