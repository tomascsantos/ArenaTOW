...
import io.github.TcFoxy.ArenaTOW.BattleArena.matches.MyMatchCancelledEvent;
import io.github.TcFoxy.ArenaTOW.BattleArena.matches.MyMatchCompletedEvent;
import io.github.TcFoxy.ArenaTOW.BattleArena.matches.MyMatchFindCurrentLeaderEvent;
...
import io.github.TcFoxy.ArenaTOW.BattleArena.matches.MyMatchPrestartEvent;
import io.github.TcFoxy.ArenaTOW.BattleArena.matches.MyMatchResultEvent;
import io.github.TcFoxy.ArenaTOW.BattleArena.matches.MyMatchStartEvent;
import io.github.TcFoxy.ArenaTOW.BattleArena.matches.MyMatchTimerIntervalEvent;
...
public abstract class MyMatch extends MyCompetition implements Runnable, ArenaController {

    public enum PlayerState{OUTOFMATCH,INMATCH}

    final MatchParams params; /// Our parameters for this match
    final MyArena arena; /// The arena we are using
    ...
    private final Collection<MyArenaPlayer> inGamePlayers = new HashSet<MyArenaPlayer>();
    ...
    final Set<MyArenaTeam> nonEndingTeams = new HashSet<MyArenaTeam>();
    final Map<MyArenaTeam,Integer> individualTeamTimers = new HashMap<MyArenaTeam,Integer>();
    final Map<MyArenaTeam,TeamTimeLimit> individualTeamTimeLimits = new HashMap<MyArenaTeam,TeamTimeLimit>();
    final Set<MyArenaTeam> deadTeams = new HashSet<MyArenaTeam>();
    ...
    public MyMatch(MyArena arena, MatchParams matchParams, Collection<ArenaListener> listeners) {
...
//        this.spawnsRandom = tinState != null && tops.hasOptionAt(tinState, TransitionOption.RANDOMSPAWN);
    ...
    private void updateBukkitEvents(MatchState matchState,MyArenaPlayer player){
     ...
     */
    public void open(){
        MyMatchOpenEvent event = new MyMatchOpenEvent(this);
        callEvent(event);
    ...
    public void hookTeamJoinHandler(AbstractJoinHandler teamJoinHandler){
        ...
        for (int i=0;i<teams.size();i++){
            ...
            for (MyArenaPlayer ap : this.teams.get(i).getPlayers()) {
                joiningOngoing(this.teams.get(i),ap);
            }
        }
        inGamePlayers.addAll(joinHandler.getPlayers());
    }
    public List<MyArenaTeam> getNonEmptyTeams() {
        List<MyArenaTeam> teams = new ArrayList<MyArenaTeam>();
        for (MyArenaTeam at: this.teams) {
            if (at != null && at.size() > 0) {
                teams.add(at);}
        }
        return teams;
    }
    private void preStartTeams(List<MyArenaTeam> teams, boolean matchPrestarting){
    ...
    private void preStartMatch() {
        ...
        callEvent(new MyMatchPrestartEvent(this,teams));
...
//        }
        List<MyArenaTeam> competingTeams = new ArrayList<MyArenaTeam>();
        /// If we will teleport them into the arena for the first time, check to see they are ready first
    ...
    private boolean checkEnoughTeams(List<MyArenaTeam> competingTeams, int neededTeams) {
        final int nCompetingTeams = competingTeams.size();
        if (nCompetingTeams >= neededTeams){
            return true;
        } else if (nCompetingTeams < neededTeams && nCompetingTeams==1){
            MyArenaTeam victor = competingTeams.get(0);
    ...
    private synchronized void nonEndingMatchWinLossOrDraw(MatchResult result){
        List<MyArenaTeam> remove = new ArrayList<MyArenaTeam>();
        for (MyArenaTeam t: result.getLosers())
            if (!nonEndingTeams.add(t) || t.size()==0)
                remove.add(t); /// they are already being handled
        result.removeLosers(remove);
        remove.clear();
        for (MyArenaTeam t: result.getDrawers())
            if (!nonEndingTeams.add(t)|| t.size()==0)
                remove.add(t); /// they are already being handled
        result.removeDrawers(remove);
        remove.clear();
        for (MyArenaTeam t: result.getVictors())
            if (!nonEndingTeams.add(t)|| t.size()==0)
                remove.add(t); /// they are already being handled
        ...
        MyMatchResultEvent event = new MyMatchResultEvent(this,result);
        callEvent(event);
        if (event.isCancelled()){
            return;
        }
    ...
    class NonEndingMatchVictory implements Runnable{
        ...
        NonEndingMatchVictory(MyMatch am, MatchResult result){this.am = am; this.result = result;}

        @Override
        public void run() {
            List<MyArenaTeam> teams = new ArrayList<MyArenaTeam>();
            final Set<MyArenaTeam> victors = result.getVictors();
            final Set<MyArenaTeam> losers = result.getLosers();
            final Set<MyArenaTeam> drawers = result.getDrawers();
    ...
    class MatchVictory implements Runnable{
        final MyMatch am;
        MatchVictory(MyMatch am){this.am = am; }

        @Override
        public void run() {
            final Set<MyArenaTeam> victors = matchResult.getVictors();
            final Set<MyArenaTeam> losers = matchResult.getLosers();
            final Set<MyArenaTeam> drawers = matchResult.getDrawers();
            if (Defaults.DEBUG_TRACE) Log.trace(am.getID(), "Match::MatchVictory():" + am + "  victors=" + victors +
                    "  losers=" + losers + "  drawers=" + drawers + " " + matchResult + " secondsToLoot=" +
                    params.getSecondsToLoot());
    ...
    private void givePrizes(MyMatch am, CompetitionResult result){
    ...
    class NonEndingMatchCompleted implements Runnable{
        final MyMatch am;
        final MatchResult result;
        final List<MyArenaTeam> teams;

        NonEndingMatchCompleted(MyMatch am, MatchResult result, List<MyArenaTeam> teams){
            this.am = am;
            this.result = result;
            this.teams = teams;
        }
        ...
        @Override
        public void run() {
            final Collection<MyArenaTeam> victors = result.getVictors();
    ...
    class MatchCompleted implements Runnable{
        ...
        MatchCompleted(MyMatch am){this.am = am;}
        @Override
        public void run() {
            ...
            currentTimer = Scheduler.scheduleSynchronousTask(new Runnable(){
                @Override
                public void run() {
                    callEvent(new MyMatchCompletedEvent(am));
    ...
    private void deconstruct(){
        ...
        callEvent(new MyMatchFinishedEvent(match));
        ...
        for (Entry<ArenaTeam,Integer> entry : individualTeamTimers.entrySet()){
            ...
            if (!teams.contains(entry.getKey())){
                performTransition(MatchState.ONCANCEL, entry.getKey(), true);
                for (MyArenaPlayer p: entry.getKey().getPlayers()){
                    p.removeCompetition(this);
                }
            }
        }
    ...
    private boolean _addedTeam(MyArenaTeam team){
        if (this.isFinished())
            return false;
    ...
    @Override
    public boolean addedTeam(MyArenaTeam team) {
    ...
    public boolean isInMatch(MyArenaPlayer player) {
        return inMatch.contains(player.getID());
    }

    private boolean addInMatch(MyArenaPlayer player) {
        boolean added = false;
        synchronized (this) {
    ...
    private boolean removeInMatch(MyArenaPlayer player) {
    ...
    private void _addedToTeam(MyArenaTeam team, MyArenaPlayer player) {
    ...
    @Override
    public boolean removedTeam(MyArenaTeam team){
     ...
     */
    @Override
    public void addedToTeam(final MyArenaTeam team, final MyArenaPlayer player) {
        if (isEnding())
            return;
    ...
    private static void doTransition(MyMatch match, MatchState state, MyArenaPlayer player, MyArenaTeam team, boolean onlyInMatch){
        if (player != null){
    ...
    private void joiningOngoing(final MyArenaTeam team, final ArenaPlayer player) {
        final MyMatch match = this;
     ...
     */
    @Override
    public void addedToTeam(MyArenaTeam team, Collection<MyArenaPlayer> players) {
        for (ArenaPlayer ap: players)
            addedToTeam(team,ap);
    }

    @Override
    public void removedFromTeam(MyArenaTeam team, Collection<MyArenaPlayer> players) {
        for (ArenaPlayer ap:players)
            privateRemovedFromTeam(team, ap);
    }

    @Override
    public void removedFromTeam(MyArenaTeam team, MyArenaPlayer player) {
        privateRemovedFromTeam(team, player);
    }

    protected void onJoin(Collection<MyArenaTeam> teams){
        if (teams == null)
            return;
        for (MyArenaTeam t: teams){
            _addedTeam(t);}
     ...
     */
    @Override
    public boolean canLeave(MyArenaPlayer p) {
     ...
     */
    @Override
    public boolean leave(MyArenaPlayer p) {
        return joinHandler != null && joinHandler.leave(p);
    }

    protected void checkAndHandleIfTeamDead(MyArenaTeam team){
    ...
    private void privateRemovedFromTeam(MyArenaTeam team, MyArenaPlayer ap){
    ...
    private void preFirstJoin(MyArenaPlayer player){
        if (Defaults.DEBUG_TRACE) Log.info(player.getName() + " -preFirstJoin  t=" + player.getTeam());
        MyArenaTeam team = getTeam(player);
...
//            psc.addMember(player, arena.getWorldGuardRegion());}
    ...
    private void postFirstJoin(MyArenaPlayer player){
        if (Defaults.DEBUG_TRACE) Log.info(player.getName() + " -preFirstJoin  t=" + player.getTeam());
        MyArenaTeam team = getTeam(player);
    ...
    public void onArenaPlayerLeaveEvent(ArenaPlayerLeaveEvent event){
        MyArenaPlayer player = event.getPlayer();
    ...
    protected void privateQuitting(ArenaPlayerLeaveEvent event) {
        MyArenaPlayer ap = event.getPlayer();
...
//    }
     ...
     */
    @Override
    public MyArena getArena() {return arena;}
...
//            addVictoryConditions();}
    ...
    @Override
    public List<MyArenaTeam> getTeams() {return teams;}
    public List<MyArenaTeam> getAliveTeams() {
        List<MyArenaTeam> alive = new ArrayList<MyArenaTeam>();
        for (MyArenaTeam t: teams){
            if (t.isDead())
                continue;
            alive.add(t);
    ...
    public Set<MyArenaPlayer> getAlivePlayers() {
    ...
    public SpawnLocation getTeamSpawn(MyArenaTeam team, boolean random){
    ...
    public SpawnLocation getWaitRoomSpawn(MyArenaTeam team, boolean random){
    ...
    public void setVictor(MyArenaPlayer p){
        MyArenaTeam t = getTeam(p);
     ...
     * @param team ArenaTeam
     */
    public synchronized void setWinner(final MyArenaTeam team){
        setVictor(team);
    }
     ...
     * @param team ArenaTeam
     */
    public synchronized void setVictor(final MyArenaTeam team){
        setVictor(new ArrayList<MyArenaTeam>(Arrays.asList(team)));
    }

    public synchronized void setVictor(final Collection<MyArenaTeam> winningTeams){
        if (individualWins){
            ...
            ArrayList<MyArenaTeam> losers= new ArrayList<MyArenaTeam>(teams);
    ...
    public synchronized void setNonEndingVictor(final MyArenaTeam team){
        MatchResult result = new MatchResult();
    ...
    public Set<MyArenaTeam> getVictors() {return matchResult.getVictors();}
    public synchronized Set<MyArenaTeam> getLosers() {return matchResult.getLosers();}
    public Set<MyArenaTeam> getDrawers() {return matchResult.getDrawers();}

    public int indexOf(MyArenaTeam t){return teams.indexOf(t);}
    ...
    protected Set<ArenaPlayer> checkReady(final MyArenaTeam t, StateOptions mo) {
    ...
    @Override
    public boolean checkReady(MyArenaPlayer p, final MyArenaTeam t, StateOptions mo, boolean announce) {
    ...
    public String getMatchInfo() {
        ...
        List<MyArenaTeam> deadTeams = new ArrayList<MyArenaTeam>();
        List<MyArenaTeam> aliveTeams = new ArrayList<MyArenaTeam>();
        for (MyArenaTeam t: teams){
            if (t.size() == 0)
                continue;
            if (t.isDead())
        ...
        for (MyArenaTeam t: aliveTeams) sb.append(t.getTeamInfo(inMatch)).append("\n");
        for (MyArenaTeam t: deadTeams) sb.append(t.getTeamInfo(inMatch)).append("\n");
        return sb.toString();
    }
    ...
    public void timeExpired() {
        MyMatchFindCurrentLeaderEvent event = new MyMatchFindCurrentLeaderEvent(this,teams, true);
        callEvent(event);
    ...
    public void intervalTick(int remaining) {
        MyMatchFindCurrentLeaderEvent event = new MyMatchFindCurrentLeaderEvent(this,teams,false);
        callEvent(event);
        callEvent(new MyMatchTimerIntervalEvent(this, remaining));
...
//    }
    ...
    public void killTeam(int teamIndex) {
        if (teams.size() <= teamIndex)
            return;
        MyArenaTeam t = teams.get(teamIndex);
        if (t == null)
            return;
        killTeam(t);
    }

    public void killTeam(MyArenaTeam t){
        for (MyArenaPlayer ap: t.getLivingPlayers()){
    ...
    public List<MyArenaPlayer> getNonLeftPlayers() {
        List<MyArenaPlayer> players = new ArrayList<MyArenaPlayer>();
        for (MyArenaTeam at: this.getTeams()){
            for (MyArenaPlayer ap: at.getPlayers()){
                if (at.hasLeft(ap))
                    continue;
                players.add(ap);
            }
