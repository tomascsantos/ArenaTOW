...
public class MyMessageUtil {
	...
	public static boolean sendMessage(final MyArenaPlayer player, final String message) {
		return sendMessage(player.getPlayer(),message);
	}

	public static boolean sendMessage(final MyArenaPlayer player, final List<String> messages) {
	...
	public static void sendMessage(Collection<MyArenaPlayer> players, String message) {
		if (message == null)
			return;
		final String msg = colorChat(message.trim());
		if (msg.isEmpty())
			return;
		for (MyArenaPlayer p: players){
	...
	public static String joinTeams(Collection<MyArenaTeam> teams, String joinStr){
		StringBuilder sb = new StringBuilder();
		boolean first = true;
	...
	public static String joinPlayers(Collection<MyArenaPlayer> players, String joinStr){
		StringBuilder sb = new StringBuilder();
		boolean first = true;
		for (MyArenaPlayer p : players){
			if (!first) sb.append(joinStr);
