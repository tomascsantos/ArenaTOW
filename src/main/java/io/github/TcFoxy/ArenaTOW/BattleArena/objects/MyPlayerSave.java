 ...
 * @author alkarin
 */
public class MyPlayerSave {
    final MyArenaPlayer player;
    ...
    public MyPlayerSave(MyArenaPlayer player) {
        this.player = player;
    }
    ...
    public void storeHealth() {
        ...
        if (MyDefaults.DEBUG_STORAGE) Log.info("storing health=" + health + " for player=" + player.getName());
    }

    public void restoreHealth() {
        if (health == null || health <= 0)
            return;
        if (MyDefaults.DEBUG_STORAGE) Log.info("restoring health=" + health+" for player=" + player.getName());
...
//        if (Defaults.DEBUG_STORAGE) Log.info("storing in match items for = " + name +" contains=" + matchitemmap.containsKey(name));
        ...
        matchItems = pinv;
        MyBAPlayerListener.restoreMatchItemsOnReenter(player, pinv);
    }
