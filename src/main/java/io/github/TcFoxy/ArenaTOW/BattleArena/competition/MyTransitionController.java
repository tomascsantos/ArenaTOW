...
import io.github.TcFoxy.ArenaTOW.BattleArena.controllers.MyArenaController;
import io.github.TcFoxy.ArenaTOW.BattleArena.controllers.MyPlayerStoreController;
import io.github.TcFoxy.ArenaTOW.BattleArena.listeners.MyPlayerHolder;
import io.github.TcFoxy.ArenaTOW.BattleArena.objects.MyArenaClass;
...
public class MyTransitionController {
     ...
     */
    public static void transition(MyPlayerHolder am, CompetitionState transition, Collection<MyArenaTeam> teams, boolean onlyInMatch){
        if (teams == null)
            return;
        boolean first = true;
        for (MyArenaTeam team: teams){
            transition(am,transition,team,onlyInMatch,first);
            first = false;
        }
    }

    public static boolean transition(MyPlayerHolder am, final CompetitionState transition, MyArenaTeam team, boolean onlyInMatch) {
        try {
    ...
    static boolean transition(MyPlayerHolder am, final CompetitionState transition, MyArenaTeam team, boolean onlyInMatch,
                              boolean performOncePerTransitionOptions) {
        final StateOptions mo = am.getParams().getStateOptions(transition);
        if (mo == null)
            return true;
        if (performOncePerTransitionOptions && (am instanceof MyArenaController)){
            MyArenaController ac = (MyArenaController) am;
...
//            }
        ...
        for (MyArenaPlayer p : team.getPlayers()){
            transition(am, transition,p,team, onlyInMatch);
        }
        return true;
    }

    public static boolean transition(final MyPlayerHolder am, final CompetitionState transition,
                                     final MyArenaPlayer player, final MyArenaTeam team, final boolean onlyInMatch) {
    ...
    private static boolean transition(final MyPlayerHolder am, final CompetitionState transition,
                                      final MyArenaPlayer player, final MyArenaTeam team, final boolean onlyInMatch,
                                      StateGraph tops) {
        if (tops == null){
            return true;}
        ...
        if (mo == null){ /// no options
            return true;}
        if (MyDefaults.DEBUG_TRANSITIONS) Log.info("-- transition "+am.getClass().getSimpleName()+"  " + transition + " p= " +player.getName() +
        ...
        if (onlyInMatch && (!insideArena && !(teleportIn || teleportRoom) ||
                am instanceof MyMatch && !((MyMatch)am).isInMatch(player) &&
        ...
        MyPlayerStoreController psc = MyPlayerStoreController.getPlayerStoreController();
...
//            if (storeAll || mo.hasOption(TransitionOption.STOREFLIGHT)){psc.storeFlight(player);}
          ...
          //  if (mo.deEnchant()) { psc.deEnchant(p);}
            ...
            if (mo.hasOption(TransitionOption.POOLMONEY) && am instanceof MyMatch) {
                prizeMoney = ((MyMatch)am).getPrizePoolMoney() * mo.getDouble(TransitionOption.POOLMONEY) /
                        team.size();
                if (prizeMoney >= 0){
            ...
            if (mo.hasOption(TransitionOption.GIVECLASS) && player.getCurrentClass() == null){
                final MyArenaClass ac = getArenaClass(mo,teamIndex);
            ...
            if (mo.hasOption(TransitionOption.CLASSENCHANTS)){
                MyArenaClass ac = player.getCurrentClass();
                if (ac != null){
            ...
            if (MyDefaults.ANNOUNCE_GIVEN_ITEMS){
        ...
        if (restoreAll || mo.hasOption(TransitionOption.RESTOREITEMS)){
            ...
            if (MyDefaults.DEBUG_TRANSITIONS)Log.info("   "+transition+" transition restoring items "+insideArena);
...
//        if (restoreAll || mo.hasOption(TransitionOption.RESTOREFLIGHT)){psc.restoreFlight(player);}
    ...
    private static void removePerms(MyArenaPlayer p, List<String> perms) {
//		if (perms == null || perms.isEmpty()) {
//        }
        /// TODO complete
    }

    private static void addPerms(MyArenaPlayer p, List<String> perms, int ticks) {
        if (perms == null || perms.isEmpty())
            return;
    ...
    private static void giveItems(final CompetitionState ms, final MyArenaPlayer p, final List<ItemStack> items,
        ...
        if (MyDefaults.DEBUG_TRANSITIONS)Log.info("   "+ms+" transition giving items to " + p.getName());
        if (items == null || items.isEmpty())
            return;
        InventoryUtil.addItemsToInventory(p.getPlayer(),items,woolTeams,color);
    }

    private static MyArenaClass getArenaClass(StateOptions mo, final int teamIndex) {
        Map<Integer, MyArenaClass> classes = mo.getClasses();
        if (classes == null)
            return null;
        if (classes.containsKey(teamIndex)){
            return classes.get(teamIndex);
        } else if (classes.containsKey(MyArenaClass.DEFAULT)){
            return classes.get(MyArenaClass.DEFAULT);
        }
        return null;
    }
