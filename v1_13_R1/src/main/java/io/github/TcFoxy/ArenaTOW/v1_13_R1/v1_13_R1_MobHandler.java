...
class v1_13_R1_MobHandler implements TOWEntityHandler{

	private HashSet<TOWEntity> entities;
	private HashSet<TOWEntity> players;

	public v1_13_R1_MobHandler() {
    ...
    private static MyEntityZombie spawnTeamZombie(World world, double x, double y, double z, Color col) {
		...
		if(col.equals(Color.RED)){
			...
			Bukkit.broadcastMessage("ERROR, v1_13_R1_MobHandler spawnTeamZombie() invalid color");
			return null;
		}
	}
	...
	private static MyEntityGolem spawnTeamGolem(World world, double x, double y, double z, Color col) {
		...
		if(col.equals(Color.BLUE)){
			...
			Bukkit.broadcastMessage("ERROR, v1_13_R1_MobHandler spawnTeamGolem() invalid color");
			return null;
		}
	 ...
	 private static MyEntityGuardian spawnTeamGuardian(World world, double x, double y, double z, Color col) {
		...
		if(col.equals(Color.RED)){
			...
			Bukkit.broadcastMessage("ERROR, v1_13_R1_MobHandler spawnTeamGuardian() invalid color");
			return null;
		}
	...
	static void clearBehavior(PathfinderGoalSelector goalSelector, PathfinderGoalSelector targetSelector){
		LinkedHashSet goalB = (LinkedHashSet) v1_13_R1_MobHandler.getPrivateField("b", PathfinderGoalSelector.class, goalSelector); goalB.clear();
		LinkedHashSet goalC = (LinkedHashSet) v1_13_R1_MobHandler.getPrivateField("c", PathfinderGoalSelector.class, goalSelector); goalC.clear();
		LinkedHashSet targetB = (LinkedHashSet) v1_13_R1_MobHandler.getPrivateField("b", PathfinderGoalSelector.class, targetSelector); targetB.clear();
        LinkedHashSet targetC = (LinkedHashSet) v1_13_R1_MobHandler.getPrivateField("c", PathfinderGoalSelector.class, targetSelector); targetC.clear();
	}
